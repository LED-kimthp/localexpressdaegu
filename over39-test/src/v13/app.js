...
let schema;
let depthBank;
let state = { phase: "loading", step: 0, answers: {}, submitted: null, submissionStatus: null, exhibitionStatus: null, fixedCheckpointSaving: false, depthGenerating: false, adaptiveGenerating: false, summaryGenerating: false, translationGenerating: false, responseId: null, language: initialLanguage, feedback: {}, referralStatus: null, researchContact: { email: "", consent: false, status: null } };
      ...
      <section class="rc2-intro-main">
        <div class="intro-hero">
          ...
          <div class="intro-disclosure" role="note">
            ...
            <p><strong>${esc(t("자발적 참여"))}</strong><span>${esc(local.introVoluntary)}</span></p>
          </div>
        ...
        <section class="open-call-secondary" aria-label="${esc(t("공모"))}">
          <div><span>2026 OPEN CALL · OPTIONAL</span><strong>${esc(t("공모"))}</strong><p>${esc(local.openCallSecondary)}</p></div>
          <button class="secondary-button" type="button" data-action="open-call">${esc(t("공모 보기"))} <span aria-hidden="true">↗</span></button>
        </section>
...
document.addEventListener("click", (event) => {
  ...
  if (target.dataset.action === "start") { state = { phase: "survey", step: 0, answers: {}, submitted: null, submissionStatus: null, exhibitionStatus: null, fixedCheckpointSaving: false, depthGenerating: false, adaptiveGenerating: false, summaryGenerating: false, translationGenerating: false, responseId: `${isRc2 ? "RC2" : "RC1"}-${crypto.randomUUID()}`, language: state.language, feedback: {}, referralStatus: null, researchContact: { email: "", consent: false, status: null } }; saveDraft(); render(true); return; }
  ...
  if (target.dataset.action === "restart") { clearDraft(); state = { phase: "intro", step: 0, answers: {}, submitted: null, submissionStatus: null, exhibitionStatus: null, fixedCheckpointSaving: false, depthGenerating: false, adaptiveGenerating: false, summaryGenerating: false, translationGenerating: false, responseId: null, language: state.language, feedback: {}, referralStatus: null, researchContact: { email: "", consent: false, status: null } }; render(true); }
});
