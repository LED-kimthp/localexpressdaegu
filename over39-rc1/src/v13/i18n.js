...
export function translate(language, text) {
  ...
  return copy[language]?.[text] || text;
}
...
export function localizeQuestion(language, item) {
  if (!item || language === "ko") return item;
  const localized = questions[language]?.[item.id];
  if (!localized) return item;
  const result = { ...item, ...localized };
  if (Array.isArray(item.options) && localized.options) {
    result.options = item.options.map((option) => Array.isArray(option)
      ? [option[0], localized.options[option[0]] || option[1]]
      : option);
  }
  return result;
}
