...
export function renderResponseDocument(document = {}) {
  const sections = array(document.sections).map((section) => {
    const body = section.id === "summary"
      ? array(section.paragraphs).map((item) => `<div class="response-document-translation"><span>${esc(item.label)}</span><p>${esc(item.text)}</p><small>${esc(item.status)}</small></div>`).join("") || `<p class="response-document-empty">${esc(responseDocumentFrame(document.display_language || document.source_language).summaryEmpty)}</p>`
