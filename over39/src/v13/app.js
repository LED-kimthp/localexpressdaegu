...
function renderParticipantContext() {
  ...
  return `${screenHeading(copy.title, copy.help)}
    <label class="field-label">${esc(copy.field || field.text)}</label>${renderChoices("CTX_FIELD", options.field, { multi: true, max: 4 })}${renderOtherInput("CTX_FIELD", copy.fieldOther)}
    <label class="field-label">${esc(copy.mode || mode.text)}</label>${renderChoices("CTX_MODE", options.participation_mode, { multi: true, max: 4 })}${renderOtherInput("CTX_MODE", copy.modeOther)}
    <label class="field-label">${esc(copy.form || form.text)}</label>${renderChoices("CTX_FORM", options.activity_form)}
    <label class="field-label">${esc(copy.unit || unit.text)}</label>${renderChoices("CTX_UNIT", options.participation_unit)}${renderOtherInput("CTX_UNIT", copy.unitOther)}`;
}
...
function renderAdaptiveCheckpoint(checkpoint) {
  ...
  const aiNotice = turn.provenance?.kind === "ai-generated"
    ? `<p class="ai-use-note" role="note">${esc(t("이 질문은 앞선 응답을 바탕으로 AI가 구성했습니다. 표현이 어색하거나 뜻이 다르게 느껴지면 편한 방식으로 답해주세요."))}</p>`
    : "";
...
function createResponse(submissionPhase = "final") {
  ...
  return {
    ...
    payload_version: isRc2 ? "over39-rc2-5anchor-plus-conditional-pilot-v0.4.6" : "over39-rc1-payload-1",
...
function renderGlobalGreetingsConnection(connection) {
  ...
  const directionSection = `<section class="connection-section receive-mail"><h2>${esc(copy.directionTitle)}</h2>${renderConnectionChoices("receive_opt_in", [["YES", copy.receiveYes], ["NO", copy.receiveNo]])}${connection.receive_opt_in === "YES" ? `<p>${esc(copy.nonResearch)}</p>${renderConnectionChoices("greeting_connection_preference", directionOptions)}${renderConnectionChoices("translation_allowed", [["YES", copy.translatedYes], ["NO", copy.translatedNo]])}<h3>${esc(copy.arrivalMethod)}</h3>${renderConnectionChoices("reply_modes", [["EMAIL_NOTICE", copy.emailNotice, copy.emailPrivate], ["MEDIATED_WEB", copy.deviceReturn, copy.deviceHelp]], { multi: true, max: 2 })}${emailSelected ? `<div class="email-delivery-card"><span>${esc(copy.emailNotice)}</span><strong>${esc(greetingSenderName)}</strong><code>${esc(greetingSenderEmail)}</code></div><label class="field-label" for="connection-email">${esc(copy.emailLabel)}</label><input id="connection-email" class="text-input text-input-single" type="email" data-connection-input="contact_email" value="${esc(connection.contact_email)}" placeholder="name@example.com" />${renderConnectionChoices("contact_permission", [["YES", copy.emailPermissionYes], ["NO", copy.emailPermissionNo]])}` : `<p class="connection-email-required">${esc(copy.emailNeeded)}</p>`}` : ""}</section>`;
  const messageSection = `<section class="connection-section message-first"><h2>${esc(copy.messageTitle)}</h2><p>${esc(copy.messageHelp)}</p>${renderConnectionChoices("message_audience", audienceOptions)}<textarea class="text-input" data-connection-input="message_text" maxlength="600" placeholder="${esc(copy.messagePlaceholder)}">${esc(messageValue)}</textarea></section>`;
  const profile = buildConnectionProfile(state.submitted || createResponse(), connection);
  const contextLabel = profile.participant_context?.kind === "EVERYDAY" ? copy.publicEveryday : profile.role ? copy.publicRole : copy.publicRecord;
...
function renderRc2Complete(response) {
  ...
  const greetingSystemCopy = submitFunctionUrl
    ? copy.greetingStored
    : copy.greetingPreparing;
  const greetingMailCopy = submitFunctionUrl
    ? `${esc(copy.greetingReadyBefore)}<b>${esc(greetingSenderName)} &lt;${esc(greetingSenderEmail)}&gt;</b>${esc(copy.greetingReadyAfter)}`
    : esc(copy.greetingPending);
...
function referralCanSave(referral = getReferral()) {
  return !referral.sent_at && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(referral.email || "").trim()) && referral.consent === "YES";
}
...
function renderReferral() {
  ...
  const statusCopy = status === "confirmed"
    ? "추천 안내를 맡겼어요. 같은 요청은 한 번만 저장됩니다."
    : status === "queued" || status === "local_only"
      ? "추천 안내 정보는 현재 이 기기에 보관합니다. 실제 이메일 발송은 서버 연결 뒤 시작합니다."
      : status === "sending" ? "추천 안내 요청을 저장하고 있어요." : "";
  const completed = ["confirmed", "queued", "local_only"].includes(status);
  return `<main class="referral-layout"><section class="referral-main"><div class="archive-label">다음 참여자 추천</div><h1 tabindex="-1">이 프로젝트를 함께 떠올리고 싶은 사람이 있나요?</h1><p class="connection-lead">추천받은 사람에게 〈만 39세 이상〉 프로젝트와 참여 링크를 이메일로 안내합니다. 추천은 안부의 좌표와 분리된 초대 기능이며, 참여 여부는 추천받은 사람이 직접 정합니다.</p><div class="referral-fields"><label><span>이름 또는 활동명 <small>선택</small></span><input type="text" data-referral-input="name" value="${esc(referral.name)}" ${completed ? "disabled" : ""} /></label><label><span>이메일 주소 <b>필수</b></span><input type="email" data-referral-input="email" value="${esc(referral.email)}" placeholder="name@example.com" ${completed ? "disabled" : ""} /></label><label><span>추천한 이유 <small>선택</small></span><textarea data-referral-input="reason" maxlength="500" placeholder="함께 나누고 싶은 이유를 짧게 적어주세요." ${completed ? "disabled" : ""}>${esc(referral.reason)}</textarea></label></div><div class="referral-options"><label class="final-check"><input type="checkbox" data-referral-check="show_referrer" ${referral.show_referrer === "YES" ? "checked" : ""} ${completed ? "disabled" : ""} /><span>안내 메일에 추천자 표기 ‘${esc(referrer)}’를 함께 전합니다.</span></label><label class="final-check"><input type="checkbox" data-referral-check="consent" ${referral.consent === "YES" ? "checked" : ""} ${completed ? "disabled" : ""} /><span>입력한 이메일을 프로젝트 참여 안내에 사용하는 내용을 확인했어요.</span></label></div>${statusCopy ? `<p class="referral-status" role="status">${esc(statusCopy)}</p>` : ""}<div class="survey-actions"><button class="secondary-button" type="button" data-action="back-to-result">참여 기록으로 돌아가기</button><button class="primary-button" type="button" data-action="save-referral" ${referralCanSave(referral) && status !== "sending" ? "" : "disabled"}>${completed ? "추천 안내를 맡겼어요 ✓" : status === "sending" ? "처리 중…" : "추천 안내 요청하기"} <span aria-hidden="true">→</span></button></div></section></main>`;
}
...
document.addEventListener("click", (event) => {
  ...
  if (target.dataset.action === "save-referral") {
    ...
    const payload = {
      ...
      created_at: referral.requested_at || new Date().toISOString(),
    };
    referral.requested_at = payload.created_at;
